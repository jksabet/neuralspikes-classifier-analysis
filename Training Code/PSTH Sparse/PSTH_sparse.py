...
def true_pred(bin_size, event, test_trial_index, template_dict, rrm, pre_time=pretime, post_time=posttime):
	...
	for i, neuron in enumerate(template):
		...
		for j, n in enumerate(neuron):
			# print(i, j, len(testing_trial), len(testing_trial[0]))
			# print(testing_trial)
...
# Bin size: 5 ms
with open('template_dict_sparse_5.json') as f1:
	template_dict_5ms = json.load(f1)
print('5 ms loaded.')

with open('PSTH_RRM_sparse.json') as f_rrm1:
...
for runthrough1 in range(10):
	for e in data['events']:
		rrm_event = rrm_5[e]
		for i, event_number in enumerate(rrm_event):
...
with open('PSTH_sparse_results.json', 'w') as f_out:
	json.dump(results, f_out, indent=4)

print('5 ms: completed')

# Bin size: 50 ms
with open('template_dict_sparse_50.json') as f2:
	template_dict_50ms = json.load(f2)

print('50 ms loaded.')

with open('PSTH_RRM_sparse_50.json') as f_rrm2:
...
# write results to json file
with open('PSTH_sparse_results_50.json', 'w') as f_out2:
	json.dump(results, f_out2, indent=4)

print('50 ms: completed')

# Bin size: 200 ms
with open('template_dict_sparse_200.json') as f3:
	template_dict_200ms = json.load(f3)

print('200 ms loaded.')

with open('PSTH_RRM_sparse_200.json') as f_rrm3:
...
# write results to json file
with open('PSTH_sparse_results_200.json', 'w') as f_out3:
